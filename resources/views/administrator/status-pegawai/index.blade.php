...
<nav aria-label="breadcrumb mb-0">
    <ol class="breadcrumb breadcrumb-style2">
      ...
      <li class="breadcrumb-item">
        <a href="{{route('status-pegawai.index')}}">@yield('title')</a>
      </li>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <!-- AKHIR TOMBOL -->
                            <table class="table table-hover table-responsive" id="table_status_pegawai">
                              <thead>
                                <tr>
                                  <th>#</th>
                                  <th>State Name</th>
...
<script>
    ...
    // DATATABLE
    $(document).ready(function () {
        var table = $('#table_status_pegawai').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('status-pegawai.index') }}",
            columns: [
        ...
        $("#form-tambah-edit").validate({
            submitHandler: function (form) {
                ...
                $.ajax({
                    ...
                    url: "{{ route('status-pegawai.store') }}",
                    type: "POST",
                    dataType: 'json',
                    success: function (data) {
                        ...
                        $('#table_status_pegawai').DataTable().ajax.reload(null, true);
                        Swal.fire({
                            title: 'Good job!',
    ...
    $('body').on('click', '.edit-post', function () {
        ...
        $.get('status-pegawai/' + data_id + '/edit', function (data) {
    ...
    /* Archive */
    function archiveStatusPegawai(id,is_archived){
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "{{ Route('archiveStatusPegawai') }}",
            id: $('.archive-status-pegawai'+id+'').val(),
    ...
    $(document).ready(function () {
        var table = $('#table_show_archived').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('show.archived.status.pegawai') }}",
            columns: [
                {data: null,sortable:false,
                    render: function (data, type, row, meta) {
                    return meta.row + meta.settings._iDisplayStart + 1;
    ...
    /* Unarchive */
    function unarchiveStatusPegawai(id,is_archived){
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "{{ Route('unarchiveStatusPegawai') }}",
            id: $('.unarchive-status-pegawai'+id+'').val(),
            data:{'is_archived':is_archived,'id':id},
        }).done(function(data, response) {
            $('#table_show_archived').DataTable().ajax.reload(null, true);
            $('#table_status_pegawai').DataTable().ajax.reload(null, true);
